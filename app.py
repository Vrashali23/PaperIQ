...
def detect_research_gaps_advanced(sections):
    gap_keywords = [
        "however", "future work", "limitation",
        "challenge", "lack", "needs improvement",
        "not addressed", "further research"]
...
def extract_topic_focus(text):
    keywords, _ = extract_keywords_and_domain(text, top_n=5)
    return keywords
st.markdown(""" 
<style>
.stApp {
            ...
            for file in uploaded_files:
                if file.name.endswith(".pdf"):
                    ...
                    if results:
                        ...
                        results["research_gaps"] = detect_research_gaps_advanced(results["sections"])
                        ...
                        results["contributions"] = detect_contribution(results["full_text"])
                        results["redundancy_score"] = internal_redundancy_check(results["full_text"])
...
if 'results' in st.session_state:
    ...
    tab1, tab2, tab3, tab4, tab5 ,tab6 ,tab7, tab8, tab9,tab10 = st.tabs([
        "📊 Visualizations","📑 Section Summaries","📄 Metadata","💡 Suggestions","❤️ Sentiment","📌 Keywords & Domain","Chatbot","📂 Analysis History","🧠 Advanced Insights","📊 Paper Comparison"])
    with tab1:
    ...
    with tab10:
        ...
        if "comparison_results" not in st.session_state or len(st.session_state.comparison_results) < 2:
            ...
            max_score = max(
                paper.get("scores", {}).get("Composite", 0)
                for paper in papers)
            comparison_data = []
            for i, paper in enumerate(papers):
                citation_data = paper.get("citation_analysis", {})
                total_citations = (
                    citation_data.get("total_citations") or
                    citation_data.get("citation_count") or
                    citation_data.get("count") or
                    citation_data.get("citations") or 0)
